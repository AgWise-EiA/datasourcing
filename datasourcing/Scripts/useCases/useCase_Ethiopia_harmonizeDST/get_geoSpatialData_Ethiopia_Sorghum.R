...
inputDataS <- readRDS("~/agwise-datasourcing/dataops/datasourcing/Data/useCase_Ethiopia_harmonizeDST/Sorghum/raw/sorghum_AOI.RDS")

#################################################################################################################
## Given the crop growing window is defined for every zone differently we need to run the data sourcing by zone 
#################################################################################################################

for( zones in unique(inputDataS$Zone)){
  print(zones)
  zone_inputData <- inputDataS[inputDataS$Zone == zones ,]
  Planting_month_date <- unique(zone_inputData$planting_month_date)
  Harvest_month_date <- unique(zone_inputData$harvest_month_date)
  season <- 1
  plantingWindow <- 4
  
  pathOut = paste("~/agwise-datasourcing/dataops/datasourcing/Data/useCase_Ethiopia_harmonizeDST/Sorghum/result/geo_4cropModel/", zones, "/", sep="")
