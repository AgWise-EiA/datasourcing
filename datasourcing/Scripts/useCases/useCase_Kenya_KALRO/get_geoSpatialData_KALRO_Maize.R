...
#################################################################################################################
## Create the grid coordiantes for Kenay Maize
#################################################################################################################
source("~/agwise-datasourcing/dataops/datasourcing/Scripts/generic/get_GridCoordinates.R")

## Make sure the Provinces or Districts names are spelled exactly in the same way as in teh country shape file
countryShapefile <- geodata::gadm(country="Kenya", level = 2, path='.')
unique(countrySpVec$NAME_1) ## for Provinces
unique(countrySpVec$NAME_2) ## for Districts

provinces <- c("Baringo","Bomet","Bungoma","Busia","Elgeyo-Marakwet", "Embu", "Homa Bay",
...
getCoordinates(country = "Kenya", useCaseName = "KALRO", Crop = "Maize", resltn = 0.05, provinces=provinces, district = NULL)

## see teh grid coordinates created
GRidCoors <- readRDS("~/agwise-datacuration/dataops/datacuration/Data/useCase_Kenya_KALRO/Maize/result/AOI_GPS.RDS")
str(GRidCoors)
