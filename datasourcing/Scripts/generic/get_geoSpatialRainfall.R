...
crop_geoSpatial_rainfall <- function(country, useCaseName, Crop, dataSource, overwrite){
  ...
  if(dataSource == "CHIRPS"){
    pathOut <- paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Rainfall/CHIRPS", sep="")
    listRaster_rf <-list.files(path="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/chirps", pattern=".nc$")
    readLayers_rf <- terra::rast(paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/chirps", listRaster_rf, sep="/"))
    fileName <- "/CHIRPS_geospatial_Rainfall.tif"
    
  }else{
    pathOut <- paste("//home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Rainfall/AgEra", sep="")
    listRaster_rf <-list.files(path="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/AgEra", pattern=".nc$")
    readLayers_rf <- terra::rast(paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/AgEra", listRaster_rf, sep="/"))
...
#' @example summary_pointdata_rainfall(rastLayer1="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/chirps/1981.nc",
# raster2="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/chirps/1982.nc",
...
get_rf_pointData <- function(country, useCaseName, Crop, AOI = FALSE, overwrite = FALSE, 
  ...
  # Input rainfall
  if(dataSource == "CHIRPS"){
    listRaster_RF <-list.files(path="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/chirps", pattern=".nc$", full.names = TRUE)
  }else{
    listRaster_RF <-list.files(path="/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/Landing/Global_GeoData/Rainfall/AgEra", pattern=".nc$", full.names = TRUE)
  }
  
  
  # Creation of the output dir
  pathOut1 <- paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/result/Rainfall", sep="")
  pathOut2 <- paste("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Rainfall", sep="")
  pathOut3 <- paste("/home/jovyan/agwise-responsefunctions/dataops/responsefunctions/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Rainfall", sep="")
  ...
  if(AOI == TRUE){
    ...
    if (planting_harvest_sameYear ==  TRUE) {
      ...
      rf_result <- foreach(i=1:length(listRaster_RF), .packages = c('terra', 'plyr', 'stringr','tidyr')) %dopar% {
        ...
        raini$Year <- raini$harvestYear <- str_extract(rasti, "[[:digit:]]+")
  ...
  fname_rain <- ifelse(AOI == "TRUE", paste("Rainfall_pointData_AOI_", Planting_month_date, "_", dataSource, ".RDS",sep=""), paste("Rainfall_pointData_trial_", dataSource, ".RDS", sep=""))
...
get_rf_pointSummarydata <- function(country, useCaseName, Crop, AOI = FALSE, overwrite = FALSE, 
  ...
  if(AOI == TRUE){
    ...
    if (planting_harvest_sameYear ==  TRUE) {
      ...
      rf_result <- foreach(i=1:(length(listRaster_RF)-1), .packages = c('terra', 'plyr', 'stringr','tidyr')) %dopar% {
        rast1 <- listRaster_RF[i]
        source("~/agwise-datasourcing/dataops/datasourcing/Scripts/generic/get_geoSpatialRainfall.R", local = TRUE)
  ...
  fname_rain <- ifelse(AOI == "TRUE", paste("Rainfall_summaries_AOI_", Planting_month_date, "_", dataSource, ".RDS",sep=""), paste("Rainfall_summaries_trial_", dataSource, ".RDS", sep=""))
...
#' Planting_month_date = "07-01";  Harvest_month_date = "11-30";jobs=10, dataSource = "CHIRPS", scenario=TRUE)
get_rf_rasterSummarydata <- function(country, useCaseName, Crop, AOI = FALSE, overwrite = FALSE, 
  ...
  if (planting_harvest_sameYear ==  TRUE) {
    ...
    foreach(i=1:length(listRaster_RF), .packages = c('terra', 'plyr', 'stringr','tidyr'), .export='.GlobalEnv') %dopar% {
      ...
      fname_rain <- ifelse(AOI == "TRUE", paste("Rainfall_Raster_summaries_AOI_" ,Planting_month_date2, "_", dataSource,"_", listYear_CHIRPS[i], ".tif",sep=""), paste("Rainfall_Raster_summaries_Country_" ,Planting_month_date2, "_", dataSource,"_", listYear_CHIRPS[i],".tif", sep=""))
  ...
  if (AOI == FALSE & scenario == TRUE){
    listRaster_summary <- list.files(path=pathOut1, pattern=paste("Rainfall_Raster_summaries_Country_",Planting_month_date2, "_", dataSource, sep=""), full.names=T)
  }
  if (AOI == TRUE & scenario == TRUE) {
    listRaster_summary <- list.files(path=pathOut1, pattern=paste("Rainfall_Raster_summaries_AOI_", Planting_month_date2, "_", dataSource, sep=""), full.names=T)
  }
  ...
  fname_scenario <- ifelse(AOI == "TRUE", paste("Rainfall_Raster_scenario_AOI_" ,Planting_month_date2, "_", dataSource, ".tif",sep=""), paste("Rainfall_Raster_scenario_Country_" ,Planting_month_date2, "_", dataSource,".tif", sep=""))
