...
crop_geoSpatial_temp <- function(country, useCaseName, Crop, dataSource, overwrite){
  ...
  croppedLayer_tmin <- terra::crop(readLayers_tmin, countryShp)
  
   ## save result
  terra::writeRaster(croppedLayer_tmax, paste(pathOut, fileName , sep="/"), filetype="GTiff", overwrite = overwrite)
  terra::writeRaster(croppedLayer_tmin, paste(pathOut, fileName_tmin , sep="/"), filetype="GTiff", overwrite = overwrite)
...
get_temp_pointSummarydata <- function(country, useCaseName, Crop, AOI = FALSE, overwrite = FALSE, 
  ...
  pathOut2 <- paste("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Temperature", sep="")
  pathOut3 <- paste("/home/jovyan/agwise-responsefunctions/dataops/responsefunctions/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Temperature", sep="")
