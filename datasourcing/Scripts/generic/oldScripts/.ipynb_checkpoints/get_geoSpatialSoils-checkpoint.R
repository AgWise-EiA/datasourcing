...
transform_soils_data <- function(country, useCaseName, Crop, resFactor=1, overwrite = FALSE){
  ...
  pathOut <- paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/Soil", sep="")
  ...
  pathIn <- paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/Soil", sep="")
...
transform_soils_data_profile <- function(country, useCaseName, Crop, resFactor=1, overwrite = FALSE){
  ...
  depths <- c("0-5cm","5-15cm","15-30cm","30-60cm")  #to add 60-100cm, 100-200cm
  ## get soil organic matter as a function of organic carbon
  ...
  for(i in 1:length(depths)) {
    ...
    transformedLayer[[paste0("PWP_",depths[i])]] <- (transformedLayer[[paste0("PWP_",depths[i])]] + 
                                                       (0.14 * transformedLayer[[paste0("PWP_",depths[i])]] - 0.02))*100
  }
  ...
  for(i in 1:length(depths)) {
    ...
    transformedLayer[[paste0("FC_",depths[i])]] <- (transformedLayer[[paste0("FC_",depths[i])]] + (1.283 * transformedLayer[[paste0("FC_",depths[i])]]^2 - 0.374 * transformedLayer[[paste0("FC_",depths[i])]] - 0.015))*100
  ...
  for(i in 1:length(depths)) {
    ...
    transformedLayer[[paste0("SWS_",depths[i])]] <- (transformedLayer[[paste0("SWS_",depths[i])]] +(0.636*transformedLayer[[paste0("SWS_",depths[i])]]-0.107))*100
    transformedLayer[[paste0("SWS_",depths[i])]] <- (transformedLayer[[paste0("FC_",depths[i])]]/100+transformedLayer[[paste0("SWS_",depths[i])]]/100-(0.097*transformedLayer[[paste0("sand_",depths[i])]]/100)+0.043)*100
