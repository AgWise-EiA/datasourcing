  ...
  if(planting_harvest_sameYear == TRUE){
    ...
    # Compute the total  and monthly average SR 
      ...
      for(m in 1:nrow(gpsdata)){
        ...
        mrf_names <- c(paste0("solarRad_month", c(1:12)))
...
get_SolarRadiation_pointSummarydata <- function(country, useCaseName, Crop, AOI = FALSE, overwrite = FALSE, 
  ...
  pathOut2 <- paste("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/SolarRadiation", sep="")
  pathOut3 <- paste("/home/jovyan/agwise-responsefunctions/dataops/responsefunctions/Data/useCase_", country, "_",useCaseName, "/", Crop, "/raw/SolarRadiation", sep="")
  ...
  if(AOI == TRUE){
    ...
    if (planting_harvest_sameYear ==  TRUE) {
      ...
      SolarRadiation_result <- foreach(i=1:length(listRaster_SR), .packages = c('terra', 'plyr', 'stringr','tidyr')) %dopar% {
        ...
        source("~/agwise-datasourcing/dataops/datasourcing/Scripts/generic/get_geoSpatialSolarRadiation_MC.R", local = TRUE)
